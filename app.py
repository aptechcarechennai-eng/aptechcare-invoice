...
def render_documents(doc_type):
    ...
    if st.session_state.get("selected_invoice"):
        ...
        if not rows: rows=f'<tr><td colspan="4" style="padding:14px;text-align:right;color:#6b7280;">Invoice Total</td><td style="padding:14px;text-align:right;font-weight:700;">₹{total:,.0f}</td></tr>'
        logo_b64=s.get("logo_b64")
          ...
          <div style="padding:32px 40px 24px;border-bottom:2px solid #f3f4f6;">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;">
              ...
              <div style="text-align:right;">
                ...
                <div style="font-size:12px;color:#9CA3AF;margin-top:8px;">Date: {fmt_date(doc["date"])}</div>
                <div style="font-size:12px;color:#9CA3AF;">Due: {fmt_date(doc["due"])}</div>
              </div>
            </div>
          </div>
        ...
        # Download invoice
        item_rows_html="".join(
            f"<tr><td style='padding:8px 12px;color:#6b7280;border-bottom:1px solid #f3f4f6;'>{i+1}</td>"
            f"<td style='padding:8px 12px;font-weight:600;border-bottom:1px solid #f3f4f6;'>{it.get('name','')}</td>"
            f"<td style='padding:8px 12px;text-align:center;border-bottom:1px solid #f3f4f6;'>{it.get('qty',1)}</td>"
            f"<td style='padding:8px 12px;text-align:right;border-bottom:1px solid #f3f4f6;'>Rs.{it.get('price',0):,}</td>"
            f"<td style='padding:8px 12px;text-align:right;font-weight:700;border-bottom:1px solid #f3f4f6;'>Rs.{it.get('amount',0):,}</td></tr>"
            for i,it in enumerate(items)
        ) or f"<tr><td colspan='4' style='padding:12px;text-align:right;color:#6b7280;'>Invoice Total</td><td style='padding:12px;text-align:right;font-weight:700;'>Rs.{total:,.0f}</td></tr>"
        logo_img=(f"<img src='data:image/png;base64,{logo_b64}' style='width:60px;height:60px;border-radius:10px;object-fit:cover;margin-bottom:8px;'>" if logo_b64 else "<div style='width:60px;height:60px;border-radius:10px;background:#4F46E5;display:flex;align-items:center;justify-content:center;margin-bottom:8px;'><span style='color:#fff;font-weight:900;font-size:16px;'>AP</span></div>")
        pdf_html=f"""<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{doc["id"]}</title>
<style>body{{font-family:Arial,sans-serif;margin:0;padding:32px;color:#111827;}}
.hdr{{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:20px;border-bottom:2px solid #f3f4f6;margin-bottom:20px;}}
.badge{{font-size:26px;font-weight:900;color:#4F46E5;}}
table{{width:100%;border-collapse:collapse;margin:16px 0;}}
th{{background:#4F46E5;color:#fff;padding:10px 12px;text-align:left;font-size:12px;}}
.bill{{background:#f8f9fa;border-radius:8px;padding:12px 16px;margin-bottom:16px;}}
.tot{{display:flex;justify-content:flex-end;margin-top:8px;}}
.ti{{width:240px;}}
.tr{{display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #e5e7eb;font-size:13px;}}
.grand{{background:#4F46E5;color:#fff;padding:10px 12px;border-radius:6px;display:flex;justify-content:space-between;margin-top:6px;font-weight:700;}}
.footer{{margin-top:20px;padding-top:14px;border-top:1px solid #e5e7eb;font-size:12px;color:#6B7280;}}
@media print{{button{{display:none!important;}}}}</style></head><body>
<div class="hdr"><div>{logo_img}
  <div style="font-size:18px;font-weight:800;">{s.get("company_name","AP Tech Care")}</div>
  <div style="font-size:12px;color:#6b7280;">{s.get("company_address1","")}</div>
  <div style="font-size:12px;color:#6b7280;">{s.get("company_phone","")}</div>
  <div style="font-size:12px;color:#6b7280;">GST: {s.get("gst_no","")}</div>
</div><div style="text-align:right;">
  <div class="badge">{cfg["header"]}</div>
  <div style="font-size:16px;font-weight:700;margin:4px 0;">{doc["id"]}</div>
  <div style="font-size:12px;color:#6b7280;">Date: {fmt_date(doc["date"])}</div>
  <div style="font-size:12px;color:#6b7280;">Due: {fmt_date(doc["due"])}</div>
</div></div>
<div class="bill"><div style="font-size:10px;font-weight:700;color:#9CA3AF;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;">Bill To</div>
<div style="font-size:15px;font-weight:700;">{doc["customer"]}</div></div>
...
<div class="tr"><span>Subtotal</span><span>Rs.{subtotal:,}</span></div>
<div class="tr"><span>GST ({tax_rate}%)</span><span>Rs.{tax:,}</span></div>
<div class="grand"><span>Total</span><span>Rs.{total:,}</span></div>
</div></div>
<div class="footer">{s.get("payment_instructions","").replace(chr(10),"<br>")}</div>
<p style="text-align:center;font-size:11px;color:#9ca3af;margin-top:16px;">Thank you! — {s.get("company_name","AP Tech Care")}</p>
</body></html>"""
        st.markdown("<div style='margin-top:12px;'></div>",unsafe_allow_html=True)
        dl,info=st.columns([1,2])
        with dl:
            st.download_button("⬇️ Download Invoice",data=pdf_html.encode("utf-8"),
                file_name=f"{doc['id']}_{doc['customer'].replace(' ','_')}.html",
                mime="text/html",type="primary",use_container_width=True)
        with info:
            st.info("📲 To share on WhatsApp:\n1. Download → Open in Chrome\n2. Ctrl+P → Save as PDF\n3. Share PDF on WhatsApp")
        return
        ...
        with tab:
            ...
            if not filtered:
                ...
                for doc in filtered:
                    ...
                    with c1: st.markdown(f'<div style="padding:4px 0;"><div style="font-weight:600;font-size:14px;color:#111827;">{doc["customer"]}</div><div style="font-size:12px;color:#9CA3AF;">{doc["id"]} • {fmt_date(doc["date"])} • Due: {fmt_date(doc["due"])}</div></div>',unsafe_allow_html=True)
                    with c2:
...
def render_cashflow():
    ...
    with tab1:
        ...
        total_out=st.session_state.get("total_expenses",45000); net=total_in-total_out
        c1,c2,c3=st.columns(3)
